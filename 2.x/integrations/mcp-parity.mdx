...
| Configure Webhooks | ✅ | ✅ | ✅ |
| Configure Apprise | ✅ | ❌ | ✅ |
| Manage Workflow Rules (incl. Ping Alerts) | ✅ | ✅ | ✅ |
| Export Results | ✅ | ✅ | ✅ |
| Manage Ping Targets | ✅ | ❌ | ❌ |
...
| **Workflow Rules** | `list-workflow-rules`, `create-workflow-rule`, `update-workflow-rule`, `delete-workflow-rule`, `toggle-workflow-rule` | `/v1/workflow-rules`, `/v1/workflow-rules/{workflowRule}/toggle` | Full management via MCP and API. Ping alert rules are workflow rules with `event: "ping"` \+ `ping_target_id` (old `/v1/ping-alerts` endpoints removed in 2.x). |
